...
export default function TemplatesPage() {
  ...
  async function handleDelete(id: string) {
    ...
    try { await templatesApi.remove(id); setTemplates((p) => p.filter((t) => t.id !== id)); }
    catch (e) { toast((e as Error).message, "error"); }
  }
    ...
    <div className="flex h-full min-h-screen">
      ...
      <div className="w-64 border-r border-gray-200 dark:border-gray-800 bg-white dark:bg-gray-900 flex flex-col">
        <div className="flex items-center justify-between px-4 py-3 border-b border-gray-100 dark:border-gray-800">
          ...
          <button onClick={() => openEdit(null)} className="p-1.5 rounded hover:bg-gray-100 dark:hover:bg-gray-800"><Plus className="w-4 h-4 text-gray-600 dark:text-gray-400" /></button>
        </div>
        <div className="flex-1 overflow-y-auto">
          {loading ? <div className="p-3 space-y-2">{[1,2].map((i) => <div key={i} className="h-10 bg-gray-100 dark:bg-gray-800 rounded animate-pulse" />)}</div> :
            templates.map((t) => (
              <button key={t.id} onClick={() => openEdit(t)} className={`w-full text-left px-4 py-3 border-b border-gray-50 dark:border-gray-800 hover:bg-gray-50 dark:hover:bg-gray-800 ${editing?.id === t.id ? "bg-indigo-50 dark:bg-indigo-900/20" : ""}`}>
        ...
        {(editing !== null || true) && (
          <div className="flex flex-col h-full p-5 space-y-4">
            <div className="flex items-center justify-between">
              ...
              {editing && <button onClick={() => handleDelete(editing.id)} className="p-1.5 rounded hover:bg-gray-100 dark:hover:bg-gray-800"><Trash2 className="w-4 h-4 text-red-500" /></button>}
            </div>
            {[
              ...
              <div key={key}>
                ...
                <input value={form[key as keyof typeof form]} onChange={(e) => setForm({ ...form, [key]: e.target.value })} placeholder={ph}
            ...
            <div className="flex-1 flex flex-col">
              ...
              <textarea value={form.body_text} onChange={(e) => setForm({ ...form, body_text: e.target.value })} placeholder="Email body…"
            ...
            <button onClick={handleSave} disabled={saving} className="self-start px-5 py-2.5 rounded-lg bg-indigo-600 hover:bg-indigo-700 text-white text-sm font-medium disabled:opacity-60">
